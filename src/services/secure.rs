...
use cryptojs_rust::{CryptoOperation, Mode, aes::{AesDecryptor, AesEncryptor}};
...
pub fn get_decrypted_string(key: &str) -> Result<String> {
    ...
    let decoded_base64 = STANDARD.decode(key).unwrap();
...
pub fn encrypt_string(plaintext: &str) -> Result<String> {
    dotenv().ok();
    let public_enc_key = env::var("PUBLIC_ENC_KEY").expect("PUBLIC_ENC_KEY must be set");
    let mut encryptor = AesEncryptor::new_256_from_password(public_enc_key.as_bytes(), Mode::CBC).unwrap();
